package com.elend.gate.admin.interceptor;
...
import com.elend.p2p.euc.ResponseUtils;
...
public class AuthInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request,
        ...
        if (uris == null || uris.size() == 0) {
            uris = sucEsbHelper.listUserResource(ReconciliationConstant.app_id,
                                                 ReconciliationConstant.app_key,
                                                 auth.getUserName());
            cache.putObject(key, uris);
            buff.append("appId=<").append(ReconciliationConstant.app_id).append(">");
            buff.append(",EsbConstant.app_key=<").append(ReconciliationConstant.app_key).append(">");
            logger.info(buff.toString());
        }
        String keyAdmin = CacheKeyConstant.PRIVILEGE_RESOURCE_KEY_PREFIX+"_admin_"
                + auth.getUserName();
        
        Boolean isAdmin = (Boolean) cache.getObject(keyAdmin);
        // 有时候可以取到KEY，但是取出来的数据为空的
        
        if (isAdmin == null ) {
            isAdmin = sucEsbHelper.getUserAppAdmin(ReconciliationConstant.app_id,
                                                 ReconciliationConstant.app_key,
                                                 auth.getUserName());
            cache.putObject(keyAdmin, isAdmin);
            
            buff.append("appId=<").append(ReconciliationConstant.app_id).append(">");
            buff.append(",isAdmin is ").append(isAdmin);
            logger.info(buff.toString());
        }
        ...
        if (uri.equals("/")||uris.contains(uri)) {//由于要增加restful的支持在web.xml里面增加一个'/'的url-mapping，因此"/"也会进入到拦截器里面，
            if (auth.getOriNickName() == null) {
                ...
                auth.saveUserName(user.getUser_id(), user.getUser_name(),
                                  user.getNick_name(),auth.isAdmin());
                UserInfoContext.clear();
                UserInfoContext.setUserId(auth.getUserId());
                UserInfoContext.setUserName(auth.getUserName());
                UserInfoContext.setNickName(user.getNick_name());
                UserInfoContext.setAdmin(isAdmin);
            } else {
                UserInfoContext.clear();
                UserInfoContext.setUserId(auth.getUserId());
                UserInfoContext.setUserName(auth.getUserName());
                UserInfoContext.setNickName(auth.getNickName());
                UserInfoContext.setAdmin(isAdmin);
            }
