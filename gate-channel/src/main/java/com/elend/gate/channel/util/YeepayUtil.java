...
public class YeepayUtil {
    private final static Logger log = LoggerFactory.getLogger(YeepayUtil.class);
    ...
    private static YeepayConfig config=null;
    
    private static YeepayConfig getConfig(){
        if(config==null){
            config=(YeepayConfig)SpringContextUtil.getBean("yeepayConfigImpl");
        }
        return config;
    }
    ...
    public static QueryResult queryByOrder(String p2_Order) {
        ...
        QueryResult qr = null;
        String p1_MerId=getConfig().getMerId();
        String queryRefundReqURL=getConfig().getQueryRefundReqURL();
        String keyValue=getConfig().getPriKey();
        ...
        try {
            ...
            responseStr = (List<String>)HttpUtils.URLGet(queryRefundReqURL, reParams);
            log.debug("End http communications.responseStr.data["
                    + responseStr + "]");
        } catch (Exception e) {
        ...
        newHmac = DigestUtil.getHmac(new String[] { qr.getR0_Cmd(),
                                             qr.getR1_Code(),
                                             qr.getR2_TrxId(),
                                             qr.getR3_Amt(), qr.getR4_Cur(),
                                             qr.getR5_Pid(),
                                             qr.getR6_Order(), qr.getR8_MP(),
                                             qr.getRb_PayStatus(),
