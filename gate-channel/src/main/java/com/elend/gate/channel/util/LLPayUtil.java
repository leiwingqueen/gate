...
public class LLPayUtil{
    ...
    private static Logger logger = LoggerFactory.getLogger(LLPayUtil.class);
    
    private static DesPropertiesEncoder encoder = new DesPropertiesEncoder();
     ...
     * @return
     */
    public static boolean checkSign(JSONObject reqObj, String rsa_public, String md5_key)
    {
        if (reqObj == null)
        {
            return false;
    ...
    {
        logger.info("进入商户[" + reqObj.getString("oid_partner")
                + "]MD5签名验证");
        if (reqObj == null)
        {
            return false;
        ...
        // 生成待签名串
        String sign_src = genSignData(reqObj);
        logger.info("商户[" + reqObj.getString("oid_partner") + "]待签名原串"
                + encoder.encode(sign_src));
        logger.info("商户[" + reqObj.getString("oid_partner") + "]签名串"
                + sign);
        ...
        try
        {
            ...
            {
                logger.info("商户[" + reqObj.getString("oid_partner")
                        + "]MD5签名验证通过");
                return true;
            } else
            {
    ...
    private static String addSignMD5(JSONObject reqObj, String md5_key)
    {
        ...
        // 生成待签名串
        String sign_src = genSignData(reqObj);
        logger.info("商户[" + reqObj.getString("oid_partner") + "]加签原串"
                + encoder.encode(sign_src));
        sign_src += "&key=" + md5_key;
        try
        {
